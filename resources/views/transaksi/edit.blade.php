    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('transaksi.update', $transaksi->id_transaksi) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $transaksi->tanggal->format('Y-m-d')) }}" 
            ...
            <div class="mb-4">
                ...
                <select name="barang_id" id="barang_id" 
                    ...
                    @foreach($barangs as $barang)
                        <option value="{{ $barang->id_barang }}" {{ old('barang_id', $transaksi->barang_id) == $barang->id_barang ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <select name="supplier_id" id="supplier_id" 
                    ...
                    @foreach($suppliers as $supplier)
                        <option value="{{ $supplier->id_supplier }}" {{ old('supplier_id', $transaksi->supplier_id) == $supplier->id_supplier ? 'selected' : '' }}>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', $transaksi->jumlah) }}" min="1"
            ...
            <div class="mb-6">
                ...
                <textarea name="keterangan" id="keterangan" rows="3" 
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $transaksi->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
